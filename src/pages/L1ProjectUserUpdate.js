...
const L1ProjectUserUpdate = () => {
  ...
  const fetchProjects = async (username, token) => {
    try {
      ...
      const response = await axios.get(
        `https://activus-hqg2c8d7gqg7bbhq.eastasia-01.azurewebsites.net/api/projects/assigned-to-user/${username}?role=L1`,
        {
          headers: { 
  ...
  const fetchUsers = async (token) => {
    try {
      const response = await axios.get(
        'https://activus-hqg2c8d7gqg7bbhq.eastasia-01.azurewebsites.net/api/auth/approved-users',
        {
          headers: { 
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json'
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await axios.put(
        `https://activus-hqg2c8d7gqg7bbhq.eastasia-01.azurewebsites.net/api/projects/update/${selectedProject.projectId}`,
        payload,
        {
          headers: { 
            Authorization: `Bearer ${token}`,
